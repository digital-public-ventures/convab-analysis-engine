...
def test_analyze_outputs_with_cached_hash(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    if len(csv_df) != len(responses_df):
        pytest.fail(f'Analysis CSV row count did not match source CSV: {len(csv_df)}/{len(responses_df)}')
    ...
    if missing:
        pytest.fail(f'Analysis CSV was missing source record_ids (missing={len(missing)})')
    extra = record_ids - source_ids
    if extra:
        pytest.fail(f'Analysis CSV contained record_ids not present in source IDs (extra={len(extra)})')
    ...
    category_fields_allowing_empty = {
        field.get('field_name', '').strip()
        for field in schema.get('categorical_fields', [])
        if field.get('field_name')
        and field.get('allow_multiple') is True
        and field.get('minItems', field.get('min_items', 0)) == 0
        and 'None or Not Applicable'
        not in {
            str(value).strip()
    ...
    completeness_checked_category_fields = [
        column for column in category_fields if column not in category_fields_allowing_empty
    ]
    for column in enum_fields_list + completeness_checked_category_fields + free_text_fields:
