...
from app.routers.jobs_runner import add_csv_results, read_cached_analysis_rows, run_analyze_job, run_tag_dedup_job
from app.routers.models import AnalyzeRequest, JobStartResponse, TagDedupRequest
...
async def analyze_dataset_endpoint(
    ...
    *,
    ...
    job = state.job_store.create_job('analyze', metadata={'content_hash': content_hash})
    poll_url, results_url = state.build_job_urls(job.job_id)

    existing_json = state.data_store.get_analyzed_json(content_hash, ANALYSIS_JSON_FILENAME)
    existing_csv = state.data_store.get_analyzed_csv(content_hash, ANALYSIS_CSV_FILENAME)
    ...
    state.create_background_task(run_analyze_job(job_id=job.job_id, request=request))
...
async def tag_fix_endpoint(
    ...
    *,
    ...
    job = state.job_store.create_job('tag_fix', metadata={'content_hash': content_hash})
    poll_url, results_url = state.build_job_urls(job.job_id)
    ...
    if deduped_csv.exists() and mappings_path.exists() and not no_cache:
        ...
        state.job_store.set_total_rows(job.job_id, total_rows)
        state.job_store.mark_completed(job.job_id)
        return JobStartResponse(
            job_id=job.job_id,
            status=job.status,
    ...
    state.create_background_task(run_tag_dedup_job(job_id=job.job_id, request=request))
