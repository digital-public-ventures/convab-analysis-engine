...
from .llm.model_config import get_model_profile
...
class SchemaGenerator:
    ...
    def __init__(
        ...
        Args:
            model_id: Model to use - short alias ('flash', 'pro') or full ID (default: flash)
            thinking_level: Thinking level for reasoning (default: MINIMAL)
            api_key: API key for Gemini (defaults to GEMINI_API_KEY env var)
            company: Company/project identifier for filenames (default: unnamed)
        """
        self.thinking_level = thinking_level
        self.company = company

        # Get model profile for rate limits
        ...
        if profile:
            ...
            # Fallback for custom model IDs not in registry
            self.model_id = model_id
            self.rate_limiter = AsyncRateLimiter(rpm=100, tpm=100_000, rpd=1_000)
            logger.warning(f"Model {model_id} not in registry, using conservative rate limits")
    ...
    async def generate_schema(self, sample_data: list[dict], use_case: str) -> dict[str, Any]:
        ...
        """
        logger.info(f"Generating schema using {self.model_id}")
        ...
        try:
            ...
            self._log_thinking(full_response)

            # Log token usage
            if usage_metadata:
                total_tokens = usage_metadata["total_tokens"]
                logger.info(f"Schema generated ({total_tokens:,} tokens used)")
    ...
    def _log_thinking(self, response: Any) -> None:
        """Log the model's thinking process if available.
        ...
        for part in parts:
            if part.thought and part.text:
                # Log abbreviated thinking process
                thought_text = part.text
                if len(thought_text) > 300:
                    thought_text = thought_text[:300] + "..."
                logger.debug(f"Model thinking: {thought_text}")
                break
    ...
    def _update_schema_index(
        ...
        logger.info(f"Updated schema index at {index_file}")
